...
def token_endpoint():
    ...
    current_app.logger.debug(
        "verification response from indieauth. me=%s, "
...
def micropub_endpoint():
    ...
    if request.method == 'GET':
        ...
        if q == 'syndicate-to':
            ...
            reply_url = url_for('views.new_post', type='reply', _external=True)
            repost_url = url_for('views.new_post', type='share', _external=True)
            like_url = url_for('views.new_post', type='like', _external=True)
            payload = {
    ...
    if not user:
        current_app.logger.warn('received valid access token for invalid user: %s', me)
        abort(401)
    ...
    with current_app.test_request_context(base_url=get_settings().site_url,
        ...
        current_app.logger.debug('received fake request %s: %s', request, request.args)
        login_user(user)
        current_app.logger.debug('successfully authenticated as user %s => %s', me, user)
        from . import views
